...
async def _extract_candidate(db: AsyncSession, payload: dict) -> None:
    """Standalone on-demand/backfill re-extraction. `link_discovery` now
    calls the same shared helper inline for every new ReviewTask, since
    nothing enqueued this job kind automatically until that change - this
    kind stays for manually re-running extraction on an existing discovery
    (e.g. after an AI Router config change)."""
    discovery = await db.scalar(
        select(Discovery)
        .where(Discovery.discovery_id == uuid.UUID(payload["discovery_id"]))
        .with_for_update()
