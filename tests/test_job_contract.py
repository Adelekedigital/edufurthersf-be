...
def test_job_allowlist_contains_documented_jobs() -> None:
    ...
    assert "harvest_parsebot" in ALLOWED_JOB_KINDS
    assert "harvest_tavily" in ALLOWED_JOB_KINDS
    assert "sync_countries" in ALLOWED_JOB_KINDS
    assert "sweep_due_jobs" in ALLOWED_JOB_KINDS
