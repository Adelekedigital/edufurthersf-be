"""Create, pause, resume, inspect, or delete the weekly Tavily harvest
schedule on QStash.

The schedule delivers a static body to the app's stable `/internal/jobs`
callback on a cron cadence; `harvest_tavily`'s dedupe_key is recomputed
...
Source.active kill switch (see scripts/manage_tavily_source.py, which stops
the harvest from doing anything even if a stray delivery arrives).

Usage (uv run python scripts/manage_tavily_schedule.py <command> ...):
    create --base-url https://your-app.example.com --token $env:QSTASH_TOKEN \\
        --cron "0 6 * * 3"   # every Wednesday 06:00 UTC, per the rotation calendar
...
def create(args: argparse.Namespace) -> int:
    ...
    body = {"kind": "harvest_tavily", "dedupe_key": "harvest_tavily:scheduled", "payload": {}}
...
def main() -> int:
    ...
    p_create.add_argument("--cron", default="0 6 * * 3", help="default: every Wednesday 06:00 UTC")
