"""Create, pause, resume, inspect, or delete the reconcile_stuck_jobs
schedule on QStash.

The schedule delivers a static body to the app's stable `/internal/jobs`
callback every 15 minutes; `reconcile_stuck_jobs`'s dedupe_key is recomputed
...
reconcile_stuck_jobs returns an abandoned claim (a worker that crashed after
claiming a job, leaving it in `running` with a lease nobody will renew) back
to the queue - without this sweep that work is never retried and never
visible as failed. Its own lease window is exactly 15 minutes
(LEASE_SECONDS, domain/jobs.py), matching this cadence.

Usage (uv run python scripts/manage_reconcile_stuck_jobs_schedule.py <command> ...):
...
def create(args: argparse.Namespace) -> int:
    ...
    body = {
        "kind": "reconcile_stuck_jobs",
        "dedupe_key": "reconcile_stuck_jobs:scheduled",
        "payload": {},
    }
