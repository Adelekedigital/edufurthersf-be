...
### Parse.bot harvest (6 marketplace APIs)

`harvest_parsebot` is a recurring job that pulls candidates from six
Parse.bot-wrapped marketplace APIs — ScholarshipPortal (every supported
destination, both degree levels), PhDScanner (funded PhD opportunities;
covers `GB`/`DE`/`FI` well, returns nothing for `CA`/`US`), Mastersportal
(every supported destination), Opportunity Desk (no country axis; the
`application_url`-not-null field is the empirically-found filter that
separates real individual grants from aggregate "roundup" posts —
see `docs/scholarship-source-options.md`), Fastweb (US-only; `featured()`
plus all 5 `Major` enum values), and CareerOneStop `.org` (US-only,
graduate-level only) — and feeds them through the exact same
`import_feed_records` pipeline as the CSV feed import, so every result lands
as an ordinary `Discovery` needing the same review before publish. Set
`PARSE_API_KEY` in the deployment platform; the SDK (`parse_apis/`, generated
by `uv run parse sync` and committed rather than gitignored, since Railway
builds from a git checkout with no network access to Parse.bot at build
time) reads it directly from the process environment.

`Source.consecutive_harvest_failures` / `last_harvest_failure_at`
(surfaced read-only via `GET /internal/admin/sources`) track, per source,
whether *every* call in a run failed — reset to 0 on any success. A source
crossing 3 consecutive fully-failed runs logs a `parsebot_source_repeatedly_failing`
warning: a crawled-data-as-API source can break silently when the target
site changes, and this makes that pattern visible instead of scattered
per-call log lines.

Two independent, deliberately separate kill switches:

- **App-level (immediate, no redeploy):** each of the six is an ordinary
  `Source` row. Deactivating one (`POST /internal/admin/sources/{id}/deactivate`,
  or `scripts/manage_parsebot_sources.py deactivate --which ...`) stops that
...
All six are unofficial, independently maintained wrappers over public site
data — Parse.bot's own listings say so explicitly — so every `Source` row
carries `authority_grade="C"`, the same tier as ScholarshipRegion: useful
for discovery breadth, never evidence on their own.

**Current deployment (development/staging, `edufurthersf-be-dev`):** all six
Sources are registered and active (`Mastersportal (via Parse.bot)`,
`Opportunity Desk (via Parse.bot)`, `Fastweb (via Parse.bot)`,
`CareerOneStop (via Parse.bot)` added 2026-09-12, alongside the original
`ScholarshipPortal (via Parse.bot)` / `PhDScanner (via Parse.bot)`). The
existing weekly schedule above already covers all six — extending
`harvest_parsebot` with more `Source` blocks needed no new schedule.

### Tavily web search discovery

`harvest_tavily` is a recurring job that searches Tavily (plain `httpx`, no
vendor SDK) for every supported destination x two query phrasings
("graduate scholarships ...", "PhD scholarships ..."), feeding results
through the same `import_feed_records` pipeline as every other harvest
source. Deliberately open, not domain-filtered: this is Tier C discovery,
the same evidentiary bucket as Parse.bot/ScholarshipRegion — reviewers
already triage every Tier-C discovery by hand, so restricting to an
allowlist would risk silently dropping a real source before a human ever
saw it (a manual spike found real signal on domains a static filter would
likely have missed — see `docs/scholarship-source-options.md`).

Set `TAVILY_API_KEY` to activate; `TAVILY_MONTHLY_SEARCH_LIMIT` (default
900, a margin under Tavily's 1000-credit monthly free tier) is enforced via
`app/infra/research_budget.py`'s `reserve_call` — the run stops early
(not fails) once the shared monthly budget is exhausted, keeping whatever
it already found. Same two-tier kill switch as Parse.bot: the `Tavily Web
Search` `Source` row (`scripts/manage_tavily_source.py`) and the QStash
schedule (`scripts/manage_tavily_schedule.py`, default every Wednesday
06:00 UTC — Parse.bot's Monday and Tavily's Wednesday are deliberately
different days so the two Tier-C harvests don't compete for reviewer
attention on the same day).

**Current deployment (development/staging, `edufurthersf-be-dev`):**
`TAVILY_API_KEY` is set, the `Tavily Web Search` Source is registered and
active, and a schedule is live, `scheduleId=scd_6pRBMiECBMnnRFyVZTEyR7YEzcBp`,
created 2026-09-12.

```powershell
railway run -s edufurthersf-be -e development -- `
  uv run python scripts/manage_tavily_schedule.py status

railway run -s edufurthersf-be -e development -- `
  uv run python scripts/manage_tavily_schedule.py pause `
  --schedule-id scd_6pRBMiECBMnnRFyVZTEyR7YEzcBp

railway run -s edufurthersf-be -e development -- `
  uv run python scripts/manage_tavily_schedule.py resume `
  --schedule-id scd_6pRBMiECBMnnRFyVZTEyR7YEzcBp
```
...
`fetch_and_persist_page` (`app/infra/source_persistence.py`) tries the
direct `httpx` fetcher first; only on a transport failure or a
blocked/error response (status >= 400), and only if `JINA_API_KEY` is
configured and `reserve_call` allows it against `JINA_MONTHLY_CALL_LIMIT`
(default 500), does it retry via Jina.ai's Reader (`r.jina.ai`) — which
renders the target server-side and returns clean markdown, satisfying the
candidate-verification standard's "a real page, actually fetched" bar.
Domain-allowlist/SSRF `ValueError`s from the direct fetcher always
propagate immediately and never trigger the fallback, so Jina can never be
used to bypass a Source's approved-domain policy. Snapshots from the
fallback are tagged `extractor_version="jina-reader-v1"` (vs. `"raw-v1"`)
so provenance stays auditable.

No schedule to activate — this is inline fallback logic inside the existing
`fetch_source_page` job and `reverify_due`'s own re-fetch path, not a
separate recurring job. **Current deployment:** `JINA_API_KEY` is set
(2026-09-12); nothing else to activate.
